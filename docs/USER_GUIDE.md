...
**每天自动发生什么**（16:30 巡检流程，第八轮起顺序）：
1. 拉最新行情 → 判断市场状态（000001.SH 20日涨幅）
2. **执行昨日登记的待执行止损（今日开盘价成交）→ 回笼资金立即可用**（先卖后买，与回测口径一致）
3. 扫描分层池信号 → 资金守卫/Top-N 筛选（见下）→ Checklist 8 项闸门 → 买入+止损同时挂
4. 今日收盘判断：价格止损 / MA10 破位 / 移动止盈（高水位回撤3%）→ 登记明日执行
5. 归档 rich 日报 `outputs/reports/YYYY-MM-DD.md`（七模块：账户总览/持仓/操作/盈亏/风险/铁律自检/**资金利用**）+ 邮件日报 + 记录过滤统计

**智能资金控制**（第八轮，`settings.yaml capital_guard` 段，可开关）：
- **资金守卫**：可用现金 < `min_cash_threshold`（默认 5000）→ 本轮跳过全部买入尝试（汇总一条告警，不再逐票刷"资金不足"）
- **Top-N 筛选**：过预筛信号数 > 可买槽数（现金/单笔预算）时，按信号评分（40%量比+30%实体占比+30%金叉新鲜度，透明代理公式）只尝试评分最高的前 `top_n`（默认 5）只
- 日报"七、资金利用"展示：止损回笼 / 同轮再投资 / 期末闲置现金；闲置超 2 倍单笔预算时提示
...
| 板块/个股冷热 | 看板"市场热力图"页 | 行业/涨跌幅/成交额三维度，点击个股看近10日详情（数据日级，随巡检更新）|
| 资金利用效率 | 巡检报告"七、资金利用" | 卖出资金同轮复用；闲置现金是否长期偏高 |
...
ai_summary:                            # AI 收盘总结（第八轮新增，纯展示层）
  enabled: true                        # 总开关（false = 整体下线，报告不含该节）
  api_base: "https://api.xiaomimimo.com/v1"
  model: "mimo-v2.5-pro"               # 官方 /v1/models 实测 id 为小写
  timeout: 20
  max_chars: 300
```

### ai_summary：AI 收盘总结说明

每个交易日 16:30 巡检完成后，调用 LLM（默认 Mimo，OpenAI 兼容协议）基于当日巡检数据生成一段 ≤300 字的四段式收盘总结（今日市场与账户 / 持仓点评 / 操作回顾 / 风险提示），追加到 `.md` 巡检报告与邮件日报的"八、AI 收盘总结"节。

- **密钥**：放项目根目录 `.env` 文件（已被 .gitignore 忽略，绝不入库）：
  `LIHU_AI_SUMMARY__API_KEY=sk-xxxx`
- **安全边界**：AI 只复述巡检数据（市场/账户/持仓/操作/风险），不参与选股、评分、下单、止损任何决策；提示词禁止预测涨跌与输出买卖指令，结尾固定"以上内容由 AI 自动生成，仅供参考，不构成投资建议。"
- **降级行为**：未配置密钥 / 网络失败 / 模型空响应 → 自动跳过，报告不含该节，巡检照常完成（日志记一条 WARNING）。
- **关闭方式**：`settings.yaml` 改 `ai_summary.enabled: false`，或环境变量 `LIHU_AI_SUMMARY__ENABLED=false`。
