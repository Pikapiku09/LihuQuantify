...
2. 执行昨日登记的待执行止损（今日开盘价成交）
3. 今日收盘判断：价格止损 / MA10 破位 → 登记明日执行
4. 扫描分层池信号 → Checklist 8 项闸门 → 买入+止损同时挂
5. 归档日报 `outputs/reports/YYYY-MM-DD.md` + 记录过滤统计
...
alert:
  email:
    ...
    to: ["xxx@qq.com"]
    send_daily_digest: true   # 每日巡检完成后发摘要邮件
```

告警场景：风控拦截 / 止损触发 / 连亏停手 / 接口异常 / 巡检异常（warn/error
即时发邮件）+ 每日巡检摘要（信号/成交/拦截/总资产/报告路径）。
